...
# build dependencies
packageInstall "clang mesa wayland udev seatd uwsm libdisplay-info libxkbcommon libinput libdrm pipewire dbus libpipewire"
...
if [ -f /usr/bin/fht-compositor ]; then
    ...
    sudo cp target/opt/fht-compositor /usr/bin/
